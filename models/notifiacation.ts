...
export async function get_notifications(status:string) {
    let notification = await db.query(SQL `SELECT waiter, item, quantity FROM notification WHERE status = ${status}`)
    return notification;
}